...
module.exports = Flagger.extend(function Asset (location, stat, load) {
  ...
  this.mime = mime[this.type] || 'text/plain'
  this.isText = /(text|json|svg)/.test(this.mime)
  ...
  setContent: function (content) {
    if (content !== this.content) {
      if (this.isText && (content instanceof Buffer)) {
  ...
  compile: function (compileOptions) {
    ...
    if (compiler) {
      ...
      if (compiled !== this.content) {
        if (this.isText && (compiled instanceof Buffer)) {
  ...
  minify: function () {
    ...
    if (minified !== this.minifiedContent) {
      if (this.isText && (minified instanceof Buffer)) {
        minified = minified.toString()
      }
      this.minifiedContent = minified
      var cache = content.cache
      if (cache) {
        var key = minified.key = content.key
        minified.cache = cache
        cache[key] = minified
      }
   ...
   */
  route: function (url) {
    return this.eachTarget('minified', function (target, content, targetUrl) {
      chug.routes.set(url || targetUrl, {
        mime: mime[target] || 'text/html',
        content: content
      })
    })
  },
    ...
    content.replace(/@use\s+(\S+)/g, function (match, spec) {
      if (spec[0] === '.') {
        ...
        spec = spec.replace(/^([^\\/]+)/, function (name) {
          var file = name + '/package'
          var pkg
          try {
            pkg = require.resolve(file)
          } catch (e) {
            var modulesDir = process.cwd() + '/node_modules/'
            try {
              pkg = require.resolve(modulesDir + file)
            } catch (e) {
              var parentDir = path.dirname(path.dirname(__dirname)) + '/'
              pkg = require.resolve(parentDir + file)
            }
          }
